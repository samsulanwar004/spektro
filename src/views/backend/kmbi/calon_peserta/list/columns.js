...
// ** Store & Actions
import { getCalonPeserta, deleteCalonPeserta } from '../store/action'
...
const handleDelete = (row) => {
  return MySwal.fire({
    ...
    if (result.value) {
      store.dispatch(deleteCalonPeserta(row.id))
    }
  })
}
      ...
      cell: row => (
        <UncontrolledDropdown>
          ...
          <DropdownMenu right>
            <DropdownItem
              tag={Link}
              to={`/kmbi/calon_peserta/save/${row.id}`}
              className='w-100'
              onClick={() => store.dispatch(getCalonPeserta(row))}
            >
