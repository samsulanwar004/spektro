...
// ** Store & Actions
import { getPartner, deletePartner } from '../store/action'
...
const handleDelete = (row) => {
  return MySwal.fire({
    ...
    if (result.value) {
      store.dispatch(deletePartner(row.id_mitra))
    }
  })
}
      ...
      cell: row => (
        <UncontrolledDropdown>
          ...
          <DropdownMenu right>
            <DropdownItem
              tag={Link}
              to={`/content/partner/save/${row.id_mitra}`}
              className='w-100'
              onClick={() => store.dispatch(getPartner(row))}
            >
