...
// ** Store & Actions
import { getAnnouncement, deleteAnnouncement } from '../store/action'
...
const handleDelete = (row) => {
  return MySwal.fire({
    ...
    if (result.value) {
      store.dispatch(deleteAnnouncement(row.id_announcement))
    }
  })
}
      ...
      cell: row => (
        <UncontrolledDropdown>
          ...
          <DropdownMenu right>
            <DropdownItem
              tag={Link}
              to={`/content/announcement/save/${row.id_announcement}`}
              className='w-100'
              onClick={() => store.dispatch(getAnnouncement(row))}
            >
